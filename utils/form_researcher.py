"""نموذج التقديم — سلم تربص تحسين المستوى"""
...
SCALE_NAME    = "تداريب تحسين المستوى - طلبة الدكتوراه"
...
def _header():
    ...
    <div class="gov-header">
      ...
      <h1>📋 طلب الانتقاء — تداريب تحسين المستوى (طلبة الدكتوراه)</h1>
...
def show_form():
    ...
    for lst in ["rs_articles","rs_interventions","rs_patents","rs_projects","rs_supervisions"]:
    ...
    rank_ok = smart_upload("وثيقة آخر ترقية في الرتبة", "rs_rank", required=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        reg = st.number_input("عدد سنوات التسجيل", 0, 10, 0, key="rs_reg")
    with c2:
        smart_upload("وثيقة التسجيل", "rs_reg_doc", required=reg > 0)
    reg_pts = reg * 2.0
    ...
    prev_n   = st.number_input(
        ...
        min_value=0, max_value=3, value=0, key="prev_f3"
    )
    ...
    award_ok  = st.checkbox("حصلت على جائزة — 5 نقطة", key="rs_award")
    award_pts = 0.0
    if award_ok:
        if smart_upload("وثيقة الجائزة", "rs_award_doc", required=True):
            award_pts = 5.0
        else:
    ...
    if _add_btn("rs_add_art"): st.session_state.tr_articles.append({}); st.rerun()
    art_pts = 0.0; del_art = []
    for i, _ in enumerate(st.session_state.tr_articles):
        ...
        with c2:
            ...
            doi   = st.text_input(f"رابط DOI", key=f"rs_art_doi_{i}", placeholder="https://doi.org/...")
        with c3:
            smart_upload(f"PDF {i+1}", f"rs_art_pdf_{i}", required=False)
            pts = ARTICLE_PTS.get(cat, 0) if doi else 0
            item_pts(pts)
            if st.button("🗑️", key=f"rs_del_art_{i}"): del_art.append(i)
        art_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if _add_btn("rs_add_int"): st.session_state.tr_interventions.append({}); st.rerun()
    int_pts = 0.0; del_int = []
    for i, _ in enumerate(st.session_state.tr_interventions):
        ...
        with c3:
            has = smart_upload(f"شهادة {i+1}", f"rs_int_cert_{i}", required=True)
            pts = INTERV_PTS.get(int_type, 0) if has else 0
            item_pts(pts)
            if st.button("🗑️", key=f"rs_del_int_{i}"): del_int.append(i)
            int_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if _add_btn("rs_add_pat"): st.session_state.tr_patents.append({}); st.rerun()
    pat_pts = 0.0; del_pat = []
    for i, _ in enumerate(st.session_state.tr_patents):
        ...
        with c3:
            has = smart_upload(f"شهادة {i+1}", f"rs_pat_cert_{i}", required=True)
            pts = 15 if has else 0
            item_pts(pts)
            if st.button("🗑️", key=f"rs_del_pat_{i}"): del_pat.append(i)
            pat_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if _add_btn("rs_add_proj"): st.session_state.tr_projects.append({}); st.rerun()
    ...
    for i, _ in enumerate(st.session_state.tr_projects):
        ...
        with c3:
            has = smart_upload(f"وثيقة {i+1}", f"rs_proj_cert_{i}", required=True)
            pts = PROJECT_PTS.get(ptype, 0) if has else 0
            item_pts(pts)
            if st.button("🗑️", key=f"rs_del_proj_{i}"): del_proj.append(i)
            proj_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if _add_btn("rs_add_sup"): st.session_state.tr_supervisions.append({}); st.rerun()
    sup_pts = 0.0; jury_n = 0; del_sup = []
    for i, _ in enumerate(st.session_state.tr_supervisions):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"اسم الطالب {i+1}", key=f"rs_sup_stud_{i}", placeholder="اسم الطالب")
            st.text_input(f"عنوان الأطروحة {i+1}", key=f"rs_sup_thesis_{i}", placeholder="عنوان الأطروحة")
        with c2:
            stype = st.selectbox("الصفة", list(SUPERV_PTS.keys()), key=f"rs_sup_type_{i}")
            st.date_input("تاريخ المناقشة", key=f"rs_sup_date_{i}")
        with c3:
            has = smart_upload(f"محضر {i+1}", f"rs_sup_cert_{i}", required=True)
            ...
            item_pts(pts)
            if st.button("🗑️", key=f"rs_del_sup_{i}"): del_sup.append(i)
            sup_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        master_n   = st.number_input("عدد مذكرات الماستر", 0, 50, 0, key="rs_master_2")
        master_pts = 0.0
        if master_n > 0:
            if smart_upload(f"محاضر مناقشة الماستر (ملف واحد لكل المذكرات)", f"rs_master_doc_2", required=True):
    ...
    with c2:
        lic_n   = st.number_input("عدد مواضيع الليسانس", 0, 100, 0, key="rs_lic_2")
        lic_pts = 0.0
        if lic_n > 0:
            if smart_upload("وثيقة إثبات من رئيس القسم (تأطير الليسانس)", f"rs_lic_doc_2", required=True):
    ...
    c1, c2 = st.columns(2)
    with c1:
        high_ok  = st.checkbox("المنصب العالي — 2 نقطة", key="rs_high")
        high_pts = 0.0
        if high_ok:
            if smart_upload("وثيقة إثبات المنصب", "rs_high_doc", required=True):
                high_pts = 2.0
            else:
                st.markdown('<div class="alert al-wn" style="font-size:.82rem;">⚠️ لن تُحتسب بدون وثيقة.</div>', unsafe_allow_html=True)
        score_line("المنصب العالي", high_pts, 2)
    with c2:
        shared_ok  = st.checkbox("التدريس في جذع مشترك — 4 نقاط", key="rs_shared_2")
        shared_pts = 0.0
        if shared_ok:
            if smart_upload("وثيقة إثبات التدريس في جذع مشترك", f"rs_shared_doc_2", required=True):
                shared_pts = 4.0
            else:
                st.markdown('<div class="alert al-wn" style="font-size:.82rem;">⚠️ لن تُحتسب بدون وثيقة.</div>', unsafe_allow_html=True)
