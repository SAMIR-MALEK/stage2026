...
def show_form():
    ...
    prev_n   = st.number_input("عدد الاستفادات السابقة", 0, 15, 0, key="rs_prev")
    ...
    c1, c2 = st.columns(2)
    with c1:
        master_n   = st.number_input("عدد مذكرات الماستر", 0, 50, 0, key="rs_master")
        master_pts = min(master_n * 1.0, 3.0)
        score_line("نقاط الماستر", master_pts, 3)
    with c2:
        lic_n   = st.number_input("عدد مواضيع الليسانس", 0, 100, 0, key="rs_lic")
    ...
    with c2:
        shared_ok  = st.checkbox("التدريس في جذع مشترك — 4 نقاط", key="rs_shared")
