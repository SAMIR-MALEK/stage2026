...
SUBMITTED_KEY = "submitted_form3"
SCALE_NAME    = "تربص تحسين المستوى - طلبة الدكتوراه"
...
def _header():
    ...
    <div class="gov-header">
      ...
      <h1>📋 طلب الانتقاء — تربص تحسين المستوى (طلبة الدكتوراه)</h1>
...
def show_form():
    ...
    admin_docs_ok = show_admin_docs("form3")
    st.markdown("---")

    for lst in ["f3_articles","f3_interventions","f3_projects","f3_supervisions",
                "f3_law_proj","f3_label_proj","f3_reviews","f3_councils","f3_bodies"]:
    ...
    c1, c2 = st.columns(2)
    with c1:
        reg_n = st.number_input("عدد سنوات التسجيل", 0, 10, 0, key="f3_reg")
    with c2:
        smart_upload("وثيقة التسجيل", "f3_reg_doc", required=reg_n > 0)
    reg_pts = reg_n * 2.0
    ...
    prev_n   = st.number_input("عدد الاستفادات في الثلاث سنوات الأخيرة", 0, 3, 0, key="f3_prev_f1")
    prev_pts = max(3.0 - prev_n, 0.0)
    score_line("نقاط الاستفادات", prev_pts, 3)
    st.markdown('</div>', unsafe_allow_html=True)
    scores["② الاستفادات السابقة"] = prev_pts

    # ②ب مشروع ناشئة في الحاضنة
    _sec("②ب", "مشروع مؤسسة ناشئة في الحاضنة الجامعية", "1 نقطة — شهادة تسجيل المشروع.")
    startup_ok = st.checkbox("لدي مشروع مؤسسة ناشئة في الحاضنة — 1 نقطة", key="f3_startup")
    startup_pts = 0.0
    if startup_ok:
        if smart_upload("شهادة تسجيل المشروع في الحاضنة", "f3_startup_doc", required=True):
            startup_pts = 1.0
    score_line("نقاط المشروع الناشئ", startup_pts, 1)
    st.markdown('</div>', unsafe_allow_html=True)
    scores["②ب مشروع ناشئة"] = startup_pts

    # ③ الجوائز وبراءات الاختراع — معيار واحد
    _sec("③", "الجوائز الدولية/الوطنية وبراءات الاختراع",
         "جوائز مرتبطة بإنجازات علمية أو براءة اختراع — 5 نقاط.")
    award_ok  = st.checkbox("حصلت على جائزة أو براءة اختراع — 10 نقاط", key="f3_award")
    award_pts = 0.0
    if award_ok:
        if smart_upload("وثيقة الجائزة أو شهادة براءة الاختراع", "f3_award_doc", required=True):
            award_pts = 5.0
        else:
            st.markdown('<div class="alert al-wn" style="font-size:.82rem;">⚠️ لن تُحتسب بدون وثيقة.</div>', unsafe_allow_html=True)
    score_line("نقاط الجوائز/براءات الاختراع", award_pts, 5)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if _add_btn("sc_add_art"): st.session_state.f3_articles.append({}); st.rerun()
    art_pts = 0.0; c_pts = 0.0; del_art = []
    ARTICLE_PTS = {"A+":20, "A":15, "B":10, "C (وطني)":5}
    for i, _ in enumerate(st.session_state.f3_articles):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"عنوان المقال {i+1}", key=f"f3_art_title_{i}", placeholder="عنوان المقال")
            st.text_input(f"اسم المجلة {i+1}", key=f"f3_art_journal_{i}", placeholder="اسم المجلة")
        with c2:
            scope = st.selectbox("النطاق", ["دولي","وطني"], key=f"f3_art_scope_{i}")
            cats  = ["A+","A","B"] if scope=="دولي" else ["C (وطني)"]
            cat   = st.selectbox("التصنيف", cats, key=f"f3_art_cat_{i}")
            doi   = st.text_input(f"رابط DOI", key=f"f3_art_doi_{i}", placeholder="https://doi.org/...")
        with c3:
            smart_upload(f"PDF {i+1}", f"f3_art_pdf_{i}", required=False)
        ...
        art_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_art): st.session_state.f3_articles.pop(i); st.rerun()
    ...
    if _add_btn("sc_add_int"): st.session_state.f3_interventions.append({}); st.rerun()
    int_pts = 0.0; del_int = []
    int_counts = {}
    for i, _ in enumerate(st.session_state.f3_interventions):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"عنوان المداخلة {i+1}", key=f"f3_int_title_{i}", placeholder="عنوان الورقة")
            st.text_input(f"اسم المؤتمر {i+1}", key=f"f3_int_conf_{i}", placeholder="اسم المؤتمر")
        with c2:
            int_type = st.selectbox("نوع المداخلة", list(INTERV_PTS.keys()), key=f"f3_int_type_{i}")
            st.date_input("تاريخ المداخلة", key=f"f3_int_date_{i}")
        with c3:
            has = smart_upload(f"شهادة {i+1}", f"f3_int_cert_{i}", required=True)
            ...
            int_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_int): st.session_state.f3_interventions.pop(i); st.rerun()
    ...
    if _add_btn("sc_add_proj"): st.session_state.f3_projects.append({}); st.rerun()
    proj_pts = 0.0; del_proj = []
    for i, _ in enumerate(st.session_state.f3_projects):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"عنوان المشروع {i+1}", key=f"f3_proj_title_{i}", placeholder="عنوان المشروع")
        with c2:
            ptype = st.selectbox("نوع المشروع", list(PROJECT_PTS.keys()), key=f"f3_proj_type_{i}")
        with c3:
            has = smart_upload(f"وثيقة {i+1}", f"f3_proj_cert_{i}", required=True)
            pts = PROJECT_PTS.get(ptype,0) if has else 0
            item_pts(pts)
            if st.button("🗑️", key=f"sc_del_proj_{i}"): del_proj.append(i)
            proj_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_proj): st.session_state.f3_projects.pop(i); st.rerun()
    ...
    if _add_btn("sc_add_sup"): st.session_state.f3_supervisions.append({}); st.rerun()
    sup_pts = 0.0; jury_n = 0; del_sup = []
    for i, _ in enumerate(st.session_state.f3_supervisions):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"اسم الطالب {i+1}", key=f"f3_sup_stud_{i}", placeholder="اسم الطالب")
            st.text_input(f"عنوان الأطروحة {i+1}", key=f"f3_sup_thesis_{i}", placeholder="عنوان الأطروحة")
        with c2:
            stype = st.selectbox("الصفة", list(SUPERV_PTS.keys()), key=f"f3_sup_type_{i}")
            st.date_input("تاريخ المناقشة", key=f"f3_sup_date_{i}")
        with c3:
            has = smart_upload(f"محضر {i+1}", f"f3_sup_cert_{i}", required=True)
            ...
            sup_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_sup): st.session_state.f3_supervisions.pop(i); st.rerun()
    ...
    if _add_btn("sc_add_law"): st.session_state.f3_law_proj.append({}); st.rerun()
    law_pts = 0.0; del_law = []
    for i, _ in enumerate(st.session_state.f3_law_proj):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"عنوان المشروع {i+1}", key=f"f3_law_title_{i}", placeholder="عنوان المشروع")
        with c2:
            st.date_input("تاريخ المناقشة", key=f"f3_law_date_{i}")
        with c3:
            has = smart_upload(f"وثيقة {i+1}", f"f3_law_cert_{i}", required=True)
            ...
            law_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_law): st.session_state.f3_law_proj.pop(i); st.rerun()
    ...
    if _add_btn("sc_add_lbl"): st.session_state.f3_label_proj.append({}); st.rerun()
    lbl_pts = 0.0; del_lbl = []
    for i, _ in enumerate(st.session_state.f3_label_proj):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"اسم المشروع {i+1}", key=f"f3_lbl_title_{i}", placeholder="اسم المشروع")
        with c2:
            lbl_type = st.selectbox("النوع", ["وسم لابل","مبتكر","ناشئة"], key=f"f3_lbl_type_{i}")
        with c3:
            has = smart_upload(f"وثيقة {i+1}", f"f3_lbl_cert_{i}", required=True)
            pts = 5.0 if has else 0
            item_pts(pts)
            if st.button("🗑️", key=f"sc_del_lbl_{i}"): del_lbl.append(i)
            lbl_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_lbl): st.session_state.f3_label_proj.pop(i); st.rerun()
    ...
    c1, c2 = st.columns(2)
    with c1:
        master_n = st.number_input("عدد مذكرات الماستر", 0, 50, 0, key="f3_master_1")
        master_pts = 0.0
        if master_n > 0:
            if smart_upload("محاضر مناقشة الماستر (ملف واحد)", "f3_master_doc_1", required=True):
                master_pts = min(master_n * 1.0, 3.0)
        score_line("نقاط الماستر", master_pts, 3)
    with c2:
        lic_n = st.number_input("عدد مواضيع الليسانس", 0, 100, 0, key="f3_lic_1")
        lic_pts = 0.0
        if lic_n > 0:
            if smart_upload("وثيقة من رئيس القسم (ليسانس)", "f3_lic_doc_1", required=True):
                lic_pts = min(lic_n * 0.5, 3.0)
        score_line("نقاط الليسانس", lic_pts, 3)
    shared_ok = st.checkbox("التدريس في جذع مشترك — 4 نقاط", key="f3_shared_1")
    shared_pts = 0.0
    if shared_ok:
        if smart_upload("وثيقة إثبات التدريس في جذع مشترك", "f3_shared_doc_1", required=True):
            shared_pts = 4.0
    ...
    if _add_btn("sc_add_rev"): st.session_state.f3_reviews.append({}); st.rerun()
    rev_pts = 0.0; rev_counts = {}; del_rev = []
    for i, _ in enumerate(st.session_state.f3_reviews):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"اسم المجلة {i+1}", key=f"f3_rev_journal_{i}", placeholder="اسم المجلة")
        with c2:
            cat = st.selectbox("صنف المجلة", ["A+","A","B","C"], key=f"f3_rev_cat_{i}")
        with c3:
            has = smart_upload(f"إثبات {i+1}", f"f3_rev_cert_{i}", required=True)
            ...
            rev_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_rev): st.session_state.f3_reviews.pop(i); st.rerun()
    ...
    with c1:
        ...
        if st.checkbox("لدي مطبوعة بيداغوجية مصادق عليها", key="f3_pub"):
            if smart_upload("وثيقة المطبوعة", "f3_pub_doc", required=True):
                pub_pts += 3.0
                if st.checkbox("المطبوعة باللغة الإنجليزية (+2ن)", key="f3_pub_en"):
                    pub_pts += 2.0
                if st.checkbox("أدرّس بها (+2ن)", key="f3_pub_teach"):
                    pub_pts += 2.0
    with c2:
        st.markdown("**كتاب بيداغوجي/علمي ISBN (5ن)**")
        if st.checkbox("لدي كتاب برقم ISBN مصادق عليه", key="f3_book"):
            if smart_upload("وثيقة الكتاب", "f3_book_doc", required=True):
                pub_pts += 5.0
                if st.checkbox("الكتاب باللغة الإنجليزية (+2ن)", key="f3_book_en"):
                    pub_pts += 2.0
    st.markdown("**Chapterbook محكم دولي (5ن)**")
    if st.checkbox("لدي Chapterbook محكم في قاعدة بيانات دولية", key="f3_chapter"):
        if smart_upload("وثيقة Chapterbook", "f3_chapter_doc", required=True):
            pub_pts += 5.0
    st.markdown("**دروس e-Learning**")
    elearn_pts = 0.0
    c1,c2,c3 = st.columns(3)
    with c1:
        if st.checkbox("دروس (2ن)", key="f3_el_cours"):
            elearn_pts += 2.0
    with c2:
        if st.checkbox("أعمال موجهة (1ن)", key="f3_el_td"):
            elearn_pts += 1.0
    with c3:
        if st.checkbox("أعمال تطبيقية (1ن)", key="f3_el_tp"):
            elearn_pts += 1.0
    if elearn_pts > 0:
        if st.checkbox("الدروس باللغة الإنجليزية (+2ن)", key="f3_el_en"):
            elearn_pts += 2.0
        smart_upload("وثيقة إثبات e-Learning", "f3_elearn_doc", required=True)
    pub_pts += elearn_pts
    ...
    if _add_btn("sc_add_council"): st.session_state.f3_councils.append({}); st.rerun()
    council_pts = 0.0; member_total = 0.0; del_council = []
    for i, _ in enumerate(st.session_state.f3_councils):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,2,1])
        with c1:
            st.text_input(f"اسم اللجنة/المجلس {i+1}", key=f"f3_council_name_{i}", placeholder="المجلس العلمي...")
        with c2:
            ctype = st.selectbox("الصفة", list(COUNCIL_PTS.keys()), key=f"f3_council_type_{i}")
        with c3:
            has = smart_upload(f"وثيقة {i+1}", f"f3_council_cert_{i}", required=True)
            ...
            council_pts += pts
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_council): st.session_state.f3_councils.pop(i); st.rerun()
    ...
    if _add_btn("sc_add_body"): st.session_state.f3_bodies.append({}); st.rerun()
    body_pts = 0.0; del_body = []
    for i, _ in enumerate(st.session_state.f3_bodies):
        st.markdown('<div class="item-block">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3,3,1])
        with c1:
            st.text_input(f"اسم الهيئة {i+1}", key=f"f3_body_name_{i}", placeholder="CDC / CATI...")
        with c2:
            has = smart_upload(f"شهادة {i+1}", f"f3_body_cert_{i}", required=True)
            if has: body_pts += 1.0
        with c3:
            st.write("")
            if st.button("🗑️", key=f"sc_del_body_{i}"): del_body.append(i)
        st.markdown('</div>', unsafe_allow_html=True)
    for i in reversed(del_body): st.session_state.f3_bodies.pop(i); st.rerun()
    ...
    high_ok = st.checkbox("أشغل منصباً عالياً — 2 نقطة", key="f3_high")
    high_pts = 0.0
    if high_ok:
        if smart_upload("وثيقة إثبات المنصب العالي", "f3_high_doc", required=True):
            high_pts = 2.0
