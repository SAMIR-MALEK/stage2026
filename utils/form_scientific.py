...
def show_form():
    ...
    _sec("③", "الاستفادات السابقة",
         "3 نقاط إذا لم تستفد — تنقص نقطة لكل استفادة — حد أقصى 3 استفادات.")
    prev_n   = st.number_input("عدد الاستفادات في الثلاث سنوات الأخيرة",
                                min_value=0, max_value=3, value=0,
                                key="tr_prev_1")
    prev_pts = max(3.0 - prev_n, 0.0)   # لا تقل عن 0 أبداً
    ...
    score_line("نقاط الجوائز", award_pts, 10)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        master_n   = st.number_input("عدد مذكرات الماستر", 0, 50, 0, key="tr_master_1")
        master_pts = 0.0
        if master_n > 0:
            if smart_upload(f"محاضر مناقشة الماستر (ملف واحد لكل المذكرات)", f"tr_master_doc_1", required=True):
    ...
    with c2:
        lic_n   = st.number_input("عدد مواضيع الليسانس", 0, 100, 0, key="tr_lic_1")
        lic_pts = 0.0
        if lic_n > 0:
            if smart_upload("وثيقة إثبات من رئيس القسم (تأطير الليسانس)", f"tr_lic_doc_1", required=True):
    ...
    with c2:
        shared_ok  = st.checkbox("التدريس في جذع مشترك — 4 نقاط", key="tr_shared_1")
        shared_pts = 0.0
        if shared_ok:
            if smart_upload("وثيقة إثبات التدريس في جذع مشترك", f"tr_shared_doc_1", required=True):
                shared_pts = 4.0
            else:
                st.markdown('<div class="alert al-wn" style="font-size:.82rem;">⚠️ لن تُحتسب بدون وثيقة.</div>', unsafe_allow_html=True)
