...
# كل صيغة مع قيمها الممكنة في الشيت
SILKS = [
...
def _parse(s):
    try:
        v = str(s).strip()
        return json.loads(v) if v and v not in ("nan","None","{}","") else {}
    except:
        return {}
...
def _load_df():
    ...
    rows = []
    for r in records:
        صيغة = str(r.get("الصيغة", r.get("scale",""))).strip()
        سلك  = str(r.get("السلك",  r.get("silk",""))).strip()
        rows.append({
...
def _filter_silk(df, vals: list) -> pd.DataFrame:
    """فلتر الصيغة — يبحث في عمودي الصيغة والسلك"""
    vals_lower = [v.lower().strip() for v in vals]
    mask = df.apply(lambda row: any(
        v in str(row["الصيغة"]).lower() or v in str(row["السلك"]).lower()
        for v in vals_lower
    ), axis=1)
    return df[mask].copy()
...
def show_committee():
    ...
    for tab, (tab_name, silk_vals) in zip(tabs[:-1], SILKS):
        with tab:
            silk_df = _filter_silk(df, silk_vals)
    ...
    with tabs[-1]:
        ...
        for tab_name, silk_vals in SILKS:
            silk_ex = _filter_silk(df, silk_vals)
            if not silk_ex.empty:
                st.markdown(f"**{tab_name}**")
                st.dataframe(silk_ex[show], use_container_width=True)
        st.markdown("---")
        c1,c2 = st.columns(2)
        with c1:
            st.download_button(
                "📥 تحميل CSV",
                data=df[show].to_csv(encoding="utf-8-sig",index=False).encode("utf-8-sig"),
        ...
        with c2:
            try:
                ...
                if first:  # لا بيانات — ورقة افتراضية
                    wb.active.title = "لا توجد بيانات"
